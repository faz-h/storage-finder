...
async function pushToAirtable(places, accessToken, baseId, tableId, onProgress, stage) {
  const records = places.map(place => {
    ...
    return {
      fields: {
        ...
        'Coordinates': place.geometry && place.geometry.location ? `${place.geometry.location.lat}, ${place.geometry.location.lng}` : '',
        'Asset Class': 'Storage',
        'Stage': stage || 'Newly Added',
      },
    };
  });
