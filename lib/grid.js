...
// bannedFacilities: case-sensitive substring match ("Public Storage" hits
// "Public Storage - Downtown"). bannedFacilitiesExact: the whole name must
// equal the entry (case-insensitive, trimmed), for brand names too generic
// to substring-match ("Valley Storage" must not hit "Spring Valley Storage").
function isBannedName(name, bannedFacilities, bannedFacilitiesExact) {
  if (!name) return false;
  if (bannedFacilities && bannedFacilities.some(b => name.includes(b))) return true;
  if (bannedFacilitiesExact && bannedFacilitiesExact.length > 0) {
    const norm = name.trim().toLowerCase();
    if (bannedFacilitiesExact.some(b => b.toLowerCase() === norm)) return true;
  }
  return false;
}

function preFilterPlaces(places, { bannedFacilities, bannedFacilitiesExact, bounds, requiredTypes, requireRelevantName }) {
  ...
  const hasBanned = (bannedFacilities && bannedFacilities.length > 0) ||
    (bannedFacilitiesExact && bannedFacilitiesExact.length > 0);
  if (hasBanned) {
    const beforeBanned = filtered.length;
    filtered = filtered.filter(p => !isBannedName(p.name, bannedFacilities, bannedFacilitiesExact));
