...
function preFilterPlaces(places, { bannedFacilities, bounds, requiredTypes }) {
  ...
  if (bannedFacilities && bannedFacilities.length > 0) {
    ...
    console.log(`  Pre-filter: banned names removed ${before - filtered.length} (${filtered.length} remain)`);
  }
