...
function countInBounds(results, bounds) {
  let count = 0;
  for (const p of results) {
    if (p.lat >= bounds.sw.lat && p.lat <= bounds.ne.lat &&
...
async function adaptiveGridSearch(bounds, keyword, apiKey, onProgress, cancelToken) {
  ...
  while (cellQueue.length > 0) {
    ...
    // Only subdivide when the cell itself is genuinely dense. Because the Nearby
    // Search uses rankby=distance (no radius), every cell returns up to 60 of the
    // nearest places regardless of how far away they are -- so raw result count is
    // a false "dense" signal in sparse areas. Count results that actually fall
    // inside the cell bounds; truncation is only a concern if the cell alone can
    // fill the result cap.
    const inBounds = countInBounds(results, cell);
    if (inBounds >= MAX_RESULTS_PER_SEARCH && depth < MAX_SUBDIVISION_DEPTH) {
      ...
      if (diagKm > MIN_CELL_SIZE_KM) {
        ...
        cellsSubdivided++;
        console.log(`  -> Subdivided (depth ${depth + 1}, diag ${diagKm.toFixed(1)}km, ${inBounds}/${results.length} in-bounds, ${cellQueue.length} in queue)`);
      }
    }
