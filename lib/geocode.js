...
async function getBoundsForSearch(searchType, searchValue, apiKey) {
  ...
  if (searchType === 'city') {
    ...
    const diag = haversine(
      { latitude: geo.viewport.sw.lat, longitude: geo.viewport.sw.lng },
      { latitude: geo.viewport.ne.lat, longitude: geo.viewport.ne.lng }
    ) / 1000;
    if (diag > 150) {
      throw new Error(`Google couldn't find "${searchValue}" as a city. It returned "${geo.formattedAddress}" which covers ${Math.round(diag)}km. Check your spelling or try a more specific name (e.g. "El Dorado Springs, CO").`);
    }
    return {
      bounds: geo.viewport,
